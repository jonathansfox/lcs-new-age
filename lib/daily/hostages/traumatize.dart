...
Future<int> traumatize(Creature lead, String action, int y) async {
  if (lcsRandom(lead.attribute(Attribute.heart)) > lcsRandom(3)) {
    ...
    if (gameOptions.lighterTone) {
      addparagraph(y++, 0, "${lead.name} loses Heart.");
    } else {
      reaction = [
      ...
      addparagraph(y++, 0, "${lead.name} loses Heart and $reaction.");
    }
    lead.heartDamage += 1;
    move(y++, 0);
    ...
    if (gameOptions.lighterTone) {
      setColor(red);
      addparagraph(y++, 0, "${lead.name} gains Wisdom.");
    } else {
      setColor(lightBlue);
    ...
    if (!gameOptions.lighterTone) {
      ...
      mvaddstrc(y++, 2, midGray, wandering);
      await getKey();
    }
    if (oneIn(2)) {
      if (oneIn(2)) {
    ...
    if (gameOptions.lighterTone) {
      mvaddstrx(y++, 0, "&w${lead.name} doesn't seem happy about this.");
    } else {

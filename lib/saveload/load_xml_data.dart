...
import 'package:lcs_new_age/items/armor_type_xml.dart';
...
Future<void> loadXmlData() async {
  ...
  loadingFeedback("armors.xml");
  _parseDocument(await rootBundle.loadString('assets/xml/armors.xml'));
  ...
  loadingFeedback("clips.xml");
  _parseDocument(await rootBundle.loadString('assets/xml/clips.xml'));
  ...
  loadingFeedback("masks.xml");
  _parseDocument(await rootBundle.loadString('assets/xml/masks.xml'));
}
...
void _parseDocument(String xml) {
  ...
  for (XmlElement element in root.childElements) {
    ...
    if (typeName == 'default') {
      if (root.name.local == 'masks') {
        maskPrototypeId = element.innerText;
      } else {
    ...
    switch (typeName) {
      case 'armortype':
        parseArmorType(armorTypes[id] ?? ArmorType(id), element);
      case 'loottype':
      ...
      case 'cliptype':
        parseClipType(ammoTypes[id] ?? AmmoType(id), element);
      case 'masktype':
        parseArmorType(armorTypes[id] ?? ArmorType.mask(id), element);
      default:
